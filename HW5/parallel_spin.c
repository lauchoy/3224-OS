...
Part 2
4 threads using spinlocks: 0.041890s to insert and 23.829994s to retrieve. 
4 threads using mutex: 0.093267s to insert and  24.089656s to retrieve.

Using mutexes takes longer to complete because mutexes are implemented in kernel mode
while spinlocks are implemented in user mode.
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < num_threads; i++) {
        if (pthread_create(&threads[i], NULL, put_phase, (void *)i)) {
    ...
    start = now();
    for (i = 0; i < num_threads; i++) {
        if (pthread_create(&threads[i], NULL, get_phase, (void *)i)) {
          printf("ERROR. pthread_create() fail");
          exit(-1);
        }      
    }
